...
- **多講者座談(panel)專則**:輸入為兩人以上對談時,(1) 每個講者 turn 獨立成段,段落開頭以純文字 `名字：`(全形冒號)標注歸屬 — 不加粗、不 blockquote、不 list(先例:BIM_MCP_7;此格式通過 build_book_master self_assert 且渲染為純 `<p>`);(2) **段落順序 = ASR 時序**,嚴禁依主題重新分區或重排 turn;`##`/`###` 標題只能插在時序既有的話題轉折點之間,不得為了湊主題而搬動任何 turn;(3) 嚴禁把不同講者的話合併進同一段,或把 A 的話改掛到 B 名下;歸屬不確定的 turn 標 `〔待確認〕：` 交人工,**不得臆測**。使用者提供分講者參考筆記(`reference_notes.md`)時,以其為歸屬與時序的權威比對源(F1 同精神:不信 ASR 單方)。守門:`scripts/dialogue_check.py`(§ S4.5.14 / § S6.13)。
...
- **座談順序不動**:Phase D 的 hook 只能插在 turn 內句間或 turn 之間的承接處,不得移動、合併或重排任何講者 turn(R2 多講者專則的時序鐵律在本階段依然有效)。
