...
- **`describe_images.py`**: 圖片理解(Antigravity headless)— **model 用 slug id 並在啟動時 `resolve_model()` 對 `antigravity models` 解析**:Antigravity 會下架舊 model(2026-09-01「Gemini 3.5 Flash (Medium)」整批失效、三張連續失敗直接熔斷),清單裡沒有就依偏好序自動退而求其次,一次改名不該擋掉整批
- **`insert_images.py`**: 自動插圖(§ S4.5.11)— `content_lines(skip_quote=True)` 讓**錨點行空間排除補述層**(`>` 行與 `<!-- ▍補述層開始/結束 -->` 區塊),圖只插在講者段落後、相關性計分不吃補述框的 repo metadata;**零省略驗證不開此旗標**,補述框本身照樣被納入「原內容行 1:1 不變」(§ S4.5.15)
