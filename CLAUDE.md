...
**Antigravity headless 通道(2026-07-05 引入):** `scripts/describe_images.py` 由
Claude Code 對話 agent 驅動,shell 出 `antigravity -p --model "Gemini 3.5 Flash (Medium)"`
做圖片理解 — 這走 Antigravity **自己的 OAuth login**,不是 Gemini API key,
**不受**「腳本禁打 Gemini API」守門限制(那條擋的是 API key 通道)。fallback `gemini -p` 同理。
...
5. **手機側拍照帶 EXIF orientation,任何「LLM 看圖」前必須先轉正**
   - 2026-07-05 實測:0704CC 全部 22 張側拍照 orientation=3(180°),Antigravity/Gemini 讀**原始像素**(不套 EXIF)→ 整輪看著顛倒的投影片寫描述:OCR 大多仍對,但 layout 區塊(左上/右下)鏡像錯、描述被「畫面顛倒」汙染
   - 解法已內建:`describe_images.py` 送圖前 `ImageOps.exif_transpose` 到 `.img_norm/` 暫存(原檔不動);出版壓圖 `compress_images.py` 亦會轉正。**別用 `sips -r 180` 手動轉** —— 會留 EXIF tag 導致下游再翻一次(封面顛倒事故的根因)
   - 通則:圖片進任何視覺 LLM 之前,轉正責任在**送圖的工具**,不靠模型自己適應
